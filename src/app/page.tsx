    ...
    const fetchConfig = async () => {
      try {
        const [modulesRes, imagesRes, infoRes] = await Promise.all([
          axios.get(`${(process.env.NEXT_PUBLIC_API_URL || 'https://smarttransfer-backend-production.up.railway.app').replace(/[\r\n]+/g, '').trim()}/api/tenant/modules`),
          axios.get(`${(process.env.NEXT_PUBLIC_API_URL || 'https://smarttransfer-backend-production.up.railway.app').replace(/[\r\n]+/g, '').trim()}/api/tenant/hero-images`),
          axios.get(`${(process.env.NEXT_PUBLIC_API_URL || 'https://smarttransfer-backend-production.up.railway.app').replace(/[\r\n]+/g, '').trim()}/api/tenant/info`)
        ]);
