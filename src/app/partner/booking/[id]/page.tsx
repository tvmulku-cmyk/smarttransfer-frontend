...
export default function BookingDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    useEffect(() => {
        const fetchBooking = async () => {
            try {
                ...
                const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000'}/api/transfer/bookings/${id}`, {
                    headers: {
    ...
    const handleAction = async (status: 'CONFIRMED' | 'REJECTED' | 'COMPLETED') => {
        ...
        try {
            ...
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000'}/api/transfer/bookings/${booking.id}/status`, {
                method: 'PUT',
                headers: {
