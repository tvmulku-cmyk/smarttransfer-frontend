...
const AdminUsersPage: React.FC = () => {
  ...
  const fetchUsers = async () => {
    try {
      setLoading(true);
      const res = await axios.get(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000'}/api/users`);
  ...
  const handleToggleActive = async (user: User, active: boolean) => {
    try {
      const res = await axios.patch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000'}/api/users/${user.id}/active`, {
        isActive: active,
      });
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editingUser) {
        await axios.put(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000'}/api/users/${editingUser.id}`, payload);
        message.success('Kullanıcı güncellendi');
      } else {
        await axios.post(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000'}/api/users`, payload);
